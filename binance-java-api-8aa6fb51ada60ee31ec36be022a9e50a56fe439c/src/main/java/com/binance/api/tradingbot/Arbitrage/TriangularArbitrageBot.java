...
/**
 * Triangular Arbitrage Bot - Erkennt Arbitrage-Möglichkeiten zwischen drei Währungspaaren
 * auf derselben Börse (Binance)
 * 
 * Beispiel: BTC/EUR → ETH/BTC → ETH/EUR → zurück zu EUR
 * 
 * WICHTIG: Dieser Bot führt KEINE echten Trades aus, sondern zeigt nur Berechnungen!
 */
public class TriangularArbitrageBot {
    ...
    public void analyseTriangularArbitrage(String pair1, String pair2, String pair3) {
        ...
        // System.out.println("Analysiere Paare: " + pair1 + " → " + pair2 + " → " + pair3);
        // System.out.println("==========================================\n");
        ...
        BigDecimal endkapital = berechneTriangularArbitrage(preise.get(pair1), preise.get(pair2), preise.get(pair3), pair1, pair2, pair3);
        ...
        if (gewinn.compareTo(BigDecimal.ZERO) > 0) {
    ...
    private BigDecimal berechneTriangularArbitrage(BigDecimal preis1, BigDecimal preis2, BigDecimal preis3, 
        ...
        if (pair2.startsWith("ETH") && pair2.endsWith("BTC")) {
            ...
            // System.out.printf("Schritt 2 - BTC → ETH: %.8f BTC ÷ %.8f BTC/ETH = %.8f ETH%n", 
    ...
    //     BigDecimal gewinnProzent = gewinn.divide(startBetrag, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100"));
